...
void MainLoop() {
  ...
  TextureManager background( "resource/jpg/bg.jpg",
                             gSurface->format,
                             gRenderer
                           ) ;

  float scaleX = (float)spriteTexture.GetWidth() * (float)background.GetHeight() / spriteTexture.GetHeight() ;
  SDL_Rect character = { 0, 0, scaleX, background.GetHeight() } ;
