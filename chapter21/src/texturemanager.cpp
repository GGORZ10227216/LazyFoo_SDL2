...
TextureManager::TextureManager( const std::string& text, SDL_Color textColor, TTF_Font* ttf, SDL_Renderer* r )
  : render( r ) {
  SDL_Surface* textSurface = TTF_RenderText_Solid( ttf, text.c_str(), textColor ) ;
  if ( textSurface != NULL ) {
    mTexture = SDL_Utility::LoadTexture( textSurface, render ) ;
    int queryResult = SDL_QueryTexture( mTexture,
                                        &mTextureFormat,
                                        &accessMode,
                                        &mWidth,
    ...
    if ( queryResult < 0 ) {
      std::cerr << "Failed to manage texture from surface: " << textSurface << "!" << std::endl
                << "SDL_Error: " << SDL_GetError() << std::endl ;
    } // if
...
void TextureManager::Render( const int x, const int y, SDL_Rect* src, int scale, SDL_Point* c, double angle, SDL_RendererFlip flip ) {
  SDL_Rect dst = { x, y, mWidth, mHeight } ;
  if ( src != NULL ) {
    dst.w = src->w * scale ;
    dst.h = src->h * scale ;
  ...
  // SDL_SetRenderDrawColor( render, 0xff, 0xff, 0x00, 0xFF ) ;
  // SDL_RenderDrawRect( render, &dst ) ;
  SDL_RenderCopyEx( render, mTexture, src, &dst, angle, c, flip ) ;
  return ;
} // Render()
...
void TextureManager::RenderUsingCenterPoint( const int x, const int y, SDL_Rect* src, int scale, double angle, SDL_RendererFlip flip ) {
  SDL_Rect dst = { 0, 0, mWidth, mHeight } ;
  if ( src != NULL ) {
    dst.w = src->w * scale ;
    dst.h = src->h * scale ;
  ...
  SDL_RenderCopyEx( render, mTexture, src, &dst, angle, NULL, flip ) ;
