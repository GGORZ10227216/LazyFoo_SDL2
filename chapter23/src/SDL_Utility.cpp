...
TTF_Font* SDL_Utility::LoadTureTypeFont( const char* path, uint8_t px ) {
  TTF_Font* result = TTF_OpenFont( path, px ) ;
  if ( result == NULL ) {
    std::cerr << "Error to load ttf:" << path << "TTF Error:" << TTF_GetError() << std::endl ;
  } // if
