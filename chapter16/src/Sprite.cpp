...
void Sprite::Render( int x, int y, double rotate, SDL_RendererFlip flip ) {
  if ( length == 0 )
    return ;
  ...
  srcSheet.RenderUsingCenterPoint( x, y, &( frameList.at( currentFrame ) ), 4, rotate, flip ) ;
} // Render()

void Sprite::Render( int x, int y, int frame, double rotate, SDL_RendererFlip flip ) {
  srcSheet.RenderUsingCenterPoint( x, y, &( frameList.at( frame ) ), 4, rotate, flip ) ;
} // Render()
